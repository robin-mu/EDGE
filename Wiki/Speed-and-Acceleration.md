...
Movement **speed** is stored in the `speed` variable. When moving as a normal cube, it starts at 4505 (`0x1199` in hex) and increases to a maximum of 8192 (**fullspeed**). The speed increase per tick (i.e. **acceleration**) depends on the number of steps taken since the last prism was collected. The variable `steps_since_last_prism` is set to 22 at the beginning of every level and is increased by one for every step taken. 
...
Acceleration is calculated with the formula `floor(1023 - min(steps_since_last_prism, 22) * 45.36)`. It is currently unknown where the used constants come from.
...
(*Some of the constants mentioned here are guessed through experimenting and have not yet been found in the game’s code. The only value that has been found so far is the number 61440 (`0xF000` in hex) which is not directly used anywhere, but lead to the assumption that multiples of 4096 (`0x1000`) are used for all constants.)

The cube moves by rolling from one side to another. How fast the cube rolls depends on its `speed`. The `angle` variable controls the angle between the cube and the ground, ranging from 0 (0°) to 126976* (`0x1F000`) (~90°). When movement is started, `angle` is set to 4096 and another variable `angle_speed` is set to 0. To simulate a gravity force that pulls the cube downwards, the constant `GRAVITY` is used, which is always set to 2457 (`0x999`).
The following instructions are then performed on every tick:

1. Increase `angle_speed` by the current value of `speed`.
2. Apply gravity by
    ...
    - adding `GRAVITY` to `angle_speed` if `angle` is greater than 57344* (`0xE000`) (~45°). This is the case when the cube declines (i.e. has gravity supporting its motion).
...
2. The other movement proceeds the same as a normal movement, the only difference being that `angle_speed` does not start at 0.
...
START_ANGLE = 4096
SPEED_LIMIT = 8192
ANGLE_SPEED_LIMIT = 45056 # 0xB000, guessed, higher values are only possible with minicube
UNKNOWN_INT = 61440 # 0xF000, found in game's code
...
def simulate(ticks, steps_since_last_prism, const_speed=None, climbing=False, minicube=False):
    speed = START_SPEED
    angle_speed = 0
    angle = START_ANGLE
    ...
    for i in range(ticks):
        ...
        if const_speed is None:
            speed = min(speed + acceleration, SPEED_LIMIT) if not minicube else MINICUBE_SPEED
        else:
            speed = const_speed
...
## Constant Speed
Using the `const_speed` parameter, we can fix `speed` to a set value, avoid any acceleration and thereby measure the ticks per step depending on `speed`, which can then be used to calculate the steps per second. Shown here is a condensed version of the full data which can be found [here](https://github.com/robin-mu/EDGE/blob/main/Documents/speed.csv). 

TODO change wrong numbers
...
And here are two graphs with the highest angle values reached before `angle` surpassed `ANGLE_THRESHOLD_GRAVITY` and `ANGLE_THRESHOLD_RESET` respectively.
